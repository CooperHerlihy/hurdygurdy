...
void hg_platform_open_window(const char* title, u32 width, u32 height, bool windowed) {
    ...
    if (windowed) {
        s_window = SDL_CreateWindow(title, (i32)width, (i32)height, SDL_WINDOW_RESIZABLE | SDL_WINDOW_VULKAN);
    } else {
        ...
        s_window = SDL_CreateWindow(title, mode[0]->w, mode[0]->h, SDL_WINDOW_FULLSCREEN | SDL_WINDOW_VULKAN);
    }
    if (s_window == NULL)
...
VkSurfaceKHR hg_platform_create_vulkan_surface(VkInstance instance) {
    ...
    bool sdl_success = SDL_Vulkan_CreateSurface(s_window, instance, NULL, &surface);
    if (!sdl_success)

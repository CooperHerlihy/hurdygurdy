...
void main() {
    ...
    float light_distance = sqrt(dot(light_relative_position, light_relative_position));

    float diffuse = max(dot(normal, light_direction), 0.0);

    float specular = 0.0;
    if (diffuse > 0.0) {
        vec3 camera_dir = normalize(in_camera - in_position);
        specular = pow(max(dot(normalize(light_direction + camera_dir), normal), 0.0), 32.0);
    }
